...
void WalkMesh::walk_in_triangle(WalkPoint const &start, glm::vec3 const &step, WalkPoint *end_, float *time_) const {
	...
	{ //project 'step' into a barycentric-coordinates direction:
		...
		step_coords = barycentric_weights(vertices[start.indices.x], vertices[start.indices.y], vertices[start.indices.z], dest);
	}
	...
	if(start.weights.x + v.x <= 0)
	{
		float t = -start.weights.x / v.x;
		if(t <= min_time)
		{
			min_time = t;
			case_i = 1;
	...
	if(start.weights.y + v.y <= 0)
	{
		float t = -start.weights.y / v.y;
		if(t <= min_time)
		{
			min_time = t;
			case_i = 2;
	...
	if(start.weights.z + v.z <= 0)
	{
		float t = -start.weights.z / v.z;
		if(t <= min_time)
		{
			min_time = t;
			case_i = 3;
	...
	switch(case_i)
	{
		case 0:
			...
			end.weights[0] = start.weights[0] + v.x;
			end.weights[1] = start.weights[1] + v.y;
			end.weights[2] = start.weights[2] + v.z;
			break;
		case 1:
			end.indices[0] = start.indices[1];
			end.indices[1] = start.indices[2];
			end.indices[2] = start.indices[0];
			end.weights[0] = start.weights[1] + min_time * v.y;
			end.weights[1] = start.weights[2] + min_time * v.z;
			end.weights[2] = 0.0f;
			break;
		case 2:
			...
			end.weights[0] = start.weights[2] + min_time * v.z;
			end.weights[1] = start.weights[0] + min_time * v.x;
			end.weights[2] = 0.0f;
			break;
		case 3:
			end.indices = start.indices;
			end.weights[0] = start.weights[0] + min_time * v.x;
			end.weights[1] = start.weights[1] + min_time * v.y;
			end.weights[2] = 0.0f;
			break;
	}
...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
	...
	if (f != next_vertex.end()) {
		...
		//TODO
		glm::vec3 const &s_pos_a = vertices[start.indices.x];
		glm::vec3 const &s_pos_b = vertices[start.indices.y];
		glm::vec3 const &s_pos_c = vertices[start.indices.z];
		glm::vec3 s_normal = glm::normalize(glm::cross(s_pos_b - s_pos_a, s_pos_c - s_pos_a));
		
		glm::vec3 const &e_pos_a = vertices[end.indices.x];
		glm::vec3 const &e_pos_b = vertices[end.indices.y];
		glm::vec3 const &e_pos_c = vertices[end.indices.z];
		glm::vec3 e_normal = glm::normalize(glm::cross(e_pos_b - e_pos_a, e_pos_c - e_pos_a));
		
		rotation = glm::rotation(s_normal, e_normal);
